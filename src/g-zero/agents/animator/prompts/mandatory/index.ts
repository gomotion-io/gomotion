type MandatoryProps = {
  fonts: string;
  useGsapTimeline?: string;
};

export const getMandatoryInstructions = (props: MandatoryProps) => `
    ...
    **4. Hugeicons-mcp-server:**
    if you need to use icons, you should use the hugeicons-mcp-server.
        *   **Use the hugeicons-mcp-server_get_platform_usage tool to get information on how to use Hugeicons MCP Server.**
        *   **Use the hugeicons-mcp-server_list_icons tool to get available icons list.**
        *   **Use the hugeicons-mcp-server_search_icons tool to search for an icon.**
